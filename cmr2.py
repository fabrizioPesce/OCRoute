...
def image_to_pdf_with_names(image_path, output_dir):
    ...
    text = pytesseract.image_to_string(Image.open(temp_file.name), config=custom_config)
...
def review_window(numbers, image_path, output_dir):
    ...
    # Iniziale disegno corretto (solo qui!)
    update_canvas_image()

    # Campi di testo per i codici
    entries = []
    ...
    scale_factor = 1.0
    img_tk = None  # img_tk serve per tenere viva la reference
    image_id = None  # Salviamo l'ID dell'immagine sul canvas

    def update_canvas_image():
        nonlocal img_tk, image_id, scale_factor

        # Resizing the original image according to the current scale_factor
        resized_img = img.resize((int(img.width * scale_factor), int(img.height * scale_factor)), Image.LANCZOS)
        img_tk = ImageTk.PhotoImage(resized_img)

        # Cancella l'immagine precedente dal canvas
        if image_id is not None:
            canvas.delete(image_id)

        # Ridisegna l'immagine e salva il nuovo ID
        image_id = canvas.create_image(0, 0, anchor=tk.NW, image=img_tk)
        canvas.configure(scrollregion=canvas.bbox("all"))

    # Iniziale renderizzazione della immagine
    update_canvas_image()
